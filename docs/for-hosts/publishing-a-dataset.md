...
5. **`bio-prov` provenance profile**, when the manifest declares the profile's conformance target in `dct:conformsTo` (see [Provenance](#provenance) below).
