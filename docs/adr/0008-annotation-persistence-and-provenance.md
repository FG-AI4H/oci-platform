...
- **2026-05-16 — Extended by [ADR-0011](./0011-annotation-sample-rejection.md).** Rejection events join the append-only event log alongside annotation events + supervisor actions; no schema change needed (the log is already extensible). Rejected samples never produce Annotation rows, so retention + persistence + signing are unaffected.
- **2026-05-16 — Extended by [ADR-0012](./0012-annotation-rights-licensing-annotator-agreement.md).** Provenance acquires two fields: `annotatorAgreementVersion` (sha256 of the signed work-for-hire / contributor-licence agreement at annotation time) and `outputLicense` (SPDX identifier inherited from the campaign config). The hash chain covers both. RTBF (GDPR Article 17) is honoured at annotator-identity level via one-way pseudonymisation; the contribution itself remains under the declared license per the signed agreement.
