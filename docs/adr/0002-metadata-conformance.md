...
- **2026-05-29 — DUO/ODRL attachment point + requester-side scope boundary (Croissant core-team guidance).** Following OCI's questions, the MLCommons Croissant core team responded (relayed via Dr. Steffen Vogler, Bayer; thread also cc Eva Keller, Luis Oala, Simao Campos):
  - **Attachment point.** Croissant supports DUO/ODRL at multiple levels: **dataset-level** for general policy, **RecordSet-level** for granular rules over specific subsets; the recommended approach is **start at dataset level, add finer levels only when necessary.** OCI adopts exactly this: the DUO `cr:dataUseTerms`, ODRL usage rules, and the BIOCroissant data-protection block (`consentBasis`, per-jurisdiction `lawfulBasis`, `anonymizationLevel`, `dataParty`, IRB approval) all attach at **dataset level** by default. RecordSet-level attachment is reserved for the case where a sub-cohort within a dataset carries distinct consent/restriction (e.g. a sensitive sub-population). This is consistent with the per-field granularity in [ADR-0010](./0010-annotation-metadata-exposure-and-blinding.md) and the RecordSet-as-sample-granularity decision in [ADR-0016](./0016-catalog-annotation-linkage.md).
  - **Requester-side scope boundary — confirmed external.** The core team confirmed that **Croissant describes datasets only**; standardising the requester's payload (intended use, IRB references, retention, redistribution intent) — "the user's side of the handshake" — is **out of Croissant's scope and is the platform's access-control responsibility: the matching algorithm and the storage of proofs** (identities, documents). This independently validates OCI's split: the dataset declares its requirements in Croissant (ODRL/DUO), while the requester side is owned by the platform — GA4GH Passport/AAI identity + visas ([ADR-0003](./0003-tiered-identity-assurance-and-access-requirements.md)), the `duo-matcher`, and signed receipts. **Decision: OCI will not seek to move the requester payload into Croissant** (it would duplicate GA4GH Passport and muddy Croissant's scope). The BuilderStatus / requester-representation work stays in GA4GH Passport.
  - **Open collaboration.** The core team offered to review the specific information OCI attaches and to "support our initiative beyond the usual." OCI pursues this on the **dataset side**: sharing the BIOCroissant v0.1 attachment shape for their recommendation and using the channel to advance BIOCroissant toward a **canonical WG-Data IRI** (which this ADR's provisional `https://oci.ai4h.net/biocroissant/v0.1#` namespace is waiting on). Tracked in the catalog/standards engagement workstream.
