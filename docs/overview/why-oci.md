...
| **Adopt an existing single-vendor catalogue** (e.g. one of HuggingFace, Kaggle, Google Dataset Search) | Vendor lock-in, no governance over compliance posture, no mandate alignment. The OCI publishes _to_ these surfaces; it doesn't outsource its own governance to them.                    |
